	...
	sealed class GazePointer : MonoBehaviour
	{
		...
		void Start()
		{
			...
			if (EventSystem.current == null)
			{
				...
				{
					m_GazeInputModule = event_system.gameObject.GetComponent<GazeInputModule>();
				}
			}
			else
			{
			...
			Log.i(LOG_TAG, "Start() GazeInputModule " + (m_GazeInputModule != null ? "existed." : "does NOT exist."), true);
		}
		...
		void Update()
		{
			...
			float calcRingWidth = m_PointerRingWidth * (1 + ((pointerFrameOffset.z / kPointerDistanceDefault) * 0.1f));
			float calcInnerCircleRadius = m_PointerCircleRadius * (1 + ((pointerFrameOffset.z / kPointerDistanceDefault) * 0.1f));
