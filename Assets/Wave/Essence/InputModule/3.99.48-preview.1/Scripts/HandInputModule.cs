	...
	public class HandInputModule : BaseInputModule
	{
		...
		protected override void OnEnable()
		{
			if (!mEnabled)
			{
				...
				if (Camera.main != null)
				{
					...
					Log.i(LOG_TAG, "OnEnable() Left pointer tracker: " + s_HandPointerTracker[HandManager.HandType.Right].gameObject.name, true);
				}
				else
				{
		...
		public override void Process()
		{
			...
			for (int i = 0; i < s_Hands.Length; i++)
			{
				...
				/// 6. Checks the hand pose data and switches the focused hand if needed.
				///    If the focused hand is switched, skip following actions.
				if (IsHandFocusSwitched())
					return;

				/// 7. The beam and pointer will become effective when pinching and uneffective when not pinching.
				/// isPinch is updated here.
				LegalizeBeamPointerOnPinch(hand);

				/// 8. Raycasts when not dragging.
				...
				/// 9. Shows the pointer when casting to an object. Hides the pointer when not casting to any object.
				...
				/// 10. If the pinch origin is invalid, do NOT send event at this frame.
				...
				if (send_event)
				{
					...
					else if (mPointerEventData[hand].eligibleForClick)
					{
						...
						{
							DEBUG("Focus hand: " + HandManager.PrimaryInput
		...
		private bool IsHandInteractable(HandManager.HandType hand)
		{
			...
			bool primary_input = ((!m_SingleInput) || (hand == HandManager.PrimaryInput));
		...
		{
			if (HandManager.PrimaryInput == HandManager.HandType.Right)
			{
				// Switch the focus hand to left.
				if ((m_HandMotion[HandManager.HandType.Left] == HandManager.HandMotion.Pinch) && (m_PinchStrength[HandManager.HandType.Left] >= m_PinchOnThreshold))
				{
					DEBUG("IsHandFocusSwitched() Set focus hand to left.");
					HandManager.PrimaryInput = HandManager.HandType.Left;
					return true;
				}
			}

			if (HandManager.PrimaryInput == HandManager.HandType.Left)
			{
				// Switch the focus hand to right.
				if ((m_HandMotion[HandManager.HandType.Right] == HandManager.HandMotion.Pinch) && (m_PinchStrength[HandManager.HandType.Right] >= m_PinchOnThreshold))
				{
					DEBUG("IsHandFocusSwitched() Set focus hand to right.");
					HandManager.PrimaryInput = HandManager.HandType.Right;
					return true;
				}
			}
