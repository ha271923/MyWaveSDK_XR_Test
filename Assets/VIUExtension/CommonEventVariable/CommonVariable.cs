...
namespace HTC.UnityPlugin.CommonEventVariable
{
    public abstract class CommonVariableHandlerBase : CommonHandler
    {
        ...
        public abstract bool IsValueChanged { get; set; }
        public abstract bool NotifyAndResetIfChanged(); // trigger OnChange event if value changed, also reset IsValueChanged flag
    ...
    {
        public static Func<T, T, bool> DefaultComparer { get; set; }
        ...
        public abstract void SetValueWithoutNotify(T value); // this func also skip comparer and force set as changed
        public abstract void OverrideOnChangeComparer(Func<T, T, bool> func);

        static CommonVariableHandler() { DefaultComparer = EqualityComparer<T>.Default.Equals; }
    }
    ...
    public static class CommonVariable
    {
        private sealed class Handler<T> : CommonVariableHandler<T>
        {
            ...
            private Func<T, T, bool> overrideEqual = DefaultComparer;
            ...
            public override bool IsValueChanged { get; set; }
            ...
            public override bool SetValue(T value)
            {
                ...
                var changed = overrideEqual != null ? !overrideEqual(previousValue, currentValue) : DefaultComparer != null ? !DefaultComparer(previousValue, currentValue) : true;
                if (changed)
                {
                    if (EnableDebugMessage || EnableAllDebugMessage || EnableAllVariableDebugMessage)
                    {
            ...
            public override void SetValueWithoutNotify(T value)
            {
                ...
                if (EnableDebugMessage || EnableAllDebugMessage || EnableAllVariableDebugMessage)
                {
                    SendDebugMessage("[CommonVariable] " + Name + "(silent) " + PreviousValueString + " => " + CurrentValueString + " (" + SetValueCount + ")");
                }
            }

            public override bool NotifyAndResetIfChanged()
            {
