    ...
    public class CommonEventAssetEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorApplication.isPlaying)
            {
                ...
                if (paramCount >= 1) { EditorGUILayout.PropertyField(paramValue1Property, new GUIContent(targetObj.EventHandlerBase.ParamType(0).Name)); }
                if (paramCount >= 2) { EditorGUILayout.PropertyField(paramValue2Property, new GUIContent(targetObj.EventHandlerBase.ParamType(1).Name)); }
                ...
                if (needFetch)
                {
                    ...
                    for (int pc = 0, pcMax = CommonEvent.MaxExistingEventsParamCount; pc < pcMax; ++pc)
                    {
                        var allEvents = CommonEvent.AllEvents(pc);
                        if (allEvents != null) { sortedHandlers.AddRange(CommonEvent.AllEvents(pc)); }
                    }
                }
