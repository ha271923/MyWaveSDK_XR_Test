	...
	public partial class LanguageSourceData
	{
        ...
        public string Import_CSV( string Category, List<string[]> CSV, eSpreadsheetUpdateMode UpdateMode = eSpreadsheetUpdateMode.Replace )
		{
            ...
            for (int i = 1, imax = CSV.Count; i < imax; ++i)
            {
                ...
                {
                    var tokenValue = Tokens[j + LanguagesStartIdx];
                    // override tokenValue
                    tokenValue = GetValue(mDict[mLangList[j]], termData.Term, tokenValue);

                    if (!string.IsNullOrEmpty(tokenValue))   // Only change the translation if there is a new value
                    {
